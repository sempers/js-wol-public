<script>
import { LOG, ERROR, FIX_TIME } from '../../logs.js'
import $store from '../store.js'
import $bus from '../../bus.js'
import TxForm from './TxForm.vue'
...
export default {
    components: {
        TxForm, ChartDialog, StatsList, AccountsList, TxList, TagsList, WolSpinner
    },
    ...
    methods: {
        async init() {
            ...
            if (response.data) {
                ...
                this.store.accounts = rawData.accounts.reject(acc => !acc._id).map(x => new AccountModel(x)).sortBy(x => x.getSort());
                this.store.transactions = rawData.transactions.reject(tx => tx.removed || tx.f41_id < 0 || !tx.date || !tx.year || (typeof tx.month == "undefined")).map(x => new TxModel(x));
        ...
        showChart() {
            if (!this.store.shownChartDialog) {
                this.store.shownChartDialog = true;
        ...
        async saveRates() {
            ...
            delete rates.date;
            try {
                await axios.post(`/api/money/rates`, rates)
                LOG("saveRates", 'Rates have been saved');
            } catch (err) {
                ERROR("saveRates", 'Error saving rates', err, 1);
            }
        },
        ...
        async recalc() {
            try {
                const response = await axios.get(`/api/money/rates`);
                if (response.data) {
                ...
                ERROR("recalc", "error while recalculating new rates", err, 1)
            }
        },
            ...
            if ($cur.isMonthView) { //-----------------------//monthly mode
                if ($acc) { //account mode
                    ...
                    this.store.normalAccounts().forEach(acc => {
                        ...
                        logs.push({name: acc.name, begin: acc.beginBalance(ym), end: acc.endBalance(ym), converted_begin: this.store.convert(acc.beginBalance(ym), acc.currency, "RUB", ym), converted_end: this.store.convert(acc.endBalance(ym), acc.currency, "RUB", ym + 1)});
                    });
                    //console.log(logs);
                }
            } else { //----------------------------------------//yearly mode
            ...
            for (let tag in tagStats) {
                ...
                if (baseTag && baseTag.budget && sum <= 0) {
                    if (type === "monthly") {
                        ...
                        spent = -Math.round(this.store.sumTxs(tx => tx.year === $cur.year && tx.tag == baseTag.name && (tx.type === "income" || tx.type === "expense"), "RUB"));
                        max = baseTag.budget;
                        percentage = Math.round(spent / baseTag.budget * 100);
                    }
                }
        ...
        writeQueryParams() {
            ...
            query.chart = this.store.shownChartDialog;
        ...
        async processRows(rows) {
            ...
            for (let value of miniTxs) {
                var tx = new TxModel({
                    ...
                    amount: value.amount,
                    dst_amount: null,
                    src: this.store.cur.account ? this.store.cur.account.name : "MAIN",
                    date: value.date,
                    rate: 1,
                    tag: value.tag,
                    tags: [],
                ...
                try {
                    const response = await axios.post(`/api/money/tx`, tx);
                    tx._id = response.data._id;
                    this.store.transactions.push(tx);
                    this.calcAll();
                    this.showMonth();                  
                } catch (err) {
                    ERROR("processRows", `Error while adding new tx id = ${tx.f41_id}`, err, 1);
                }                
            }            
            LOG("processRows","All transactions from CSV file were successfully added");            
        },
        ...
        async saveTx(tx) {
            ...
            try {
                const response = await axios.put(`/api/money/tx`, tx_clone);
                tx.isEdited = false;
                this.calcAll();
                this.showMonth();
                LOG("saveTx", `tx id = ${tx.f41_id} updated in DB`, 1);
            } catch (err) {
                ERROR("saveTx", `Error while saving tx ${tx.f41_id}`, err, 1);
            }
        },
        ...
        async addTx(ntx) {
            ...
            try {
                const response = await axios.post(`/api/money/tx`, ntx);
                ntx._id = response.data._id;
                this.store.transactions.push(ntx);
                this.calcAll();
                this.showMonth();
                $bus.$emit("tx-saved-ok");
                LOG("addTx", `new tx id = ${ntx.f41_id} added to DB`, 1);
            } catch (err) {
                ERROR("addTx", `Error while adding new tx id = ${ntx.f41_id}`, err, 1);
            }
        },
        ...
        async removeTx(tx) {
            try {
                await axios.delete(`/api/money/tx/${tx._id}`);
                this.store.transactions = this.store.transactions.reject(_tx => _tx._id == tx._id);
                this.calcAll();
                this.showMonth();
                $bus.$emit("tx-removed-ok");
                LOG("removeTx", `tx id = ${tx.f41_id} removed`,1);
            } catch (err) {
                ERROR("removeTx", `Error while removing tx id = ${tx.f41_id}`, err, 1);
            }
        },
        ...
        async addTag(bt) {
            try {
                const response = await axios.post(`/api/money/tags`, bt);
                bt._id = response.data._id;
                this.store.baseTags.push(bt);
                LOG("addTag", `Successfully saved tag ${bt.name}`, 1);
            } catch (err) {
                ERROR("addTag", `Error while saving tag ${bt.name}`, err, 1);
            }
        },
        ...
        async saveTag(bt) {
            ...
            try {
                await axios.put(`/api/money/tags`, bt);
                this.calcAll();
                this.showMonth();
                ...
                LOG('saveTag', `Successfully saved base tag ${bt.name}`, 1);
            } catch (err) {
                ERROR('saveTag', `Error while saving base tag ${bt.name}`, err, 1);
            }
        },

        async removeTag(bt) {
            try {
                await axios.delete(`/api/money/tags/${bt._id}`);
                this.store.baseTags = this.store.baseTags.reject(_bt => _bt.name == bt.name);
                this.calcAll();
                this.showMonth();
                LOG('removeTag', `tag ${bt.name} was removed successfully`, 1);
            } catch (err) {
                ERROR('removeTag', `Error while removing tag ${bt.name}`, err, 1)
            }
        },

        async renameTag(opt) {
            try {
                await axios.post(`/api/money/tags/rename`, opt);
                this.store.transactions.forEach(tx => {
                ...
                this.calcAll();
                this.showMonth();
                LOG('renameTag', `Base tag ${opt.oldName} successfully renamed to ${opt.newName}`, 1);
            } catch (err) {
                ERROR('renameTag',`Error while renaming base tag ${opt.oldName}`, err, 1);
            }
        },
        ...
        async renameAcc(opt) {
            try {
                await axios.post(`/api/money/accounts/rename`, opt);
                this.store.transactions.forEach(tx => {
                ...
                this.calcAll();
                this.showMonth();
                LOG('renameAcc', `Account ${opt.oldName} successfully renamed to ${opt.newName}`, 1);
            } catch (err) {
                ERROR('renameAcc', `Error while renaming txs account from ${opt.oldName} to ${opt.newName}`, err, 1);
            }
        },
        ...
        async saveAcc(acc) {
            try {
                await axios.put(`/api/money/accounts`, acc)
                LOG('saveAcc', `Account ${acc.name} was saved`, 1);
            } catch (err) {
                ERROR('saveAcc', `Error while saving account ${acc.name}`, err, 1);
            }
        }
    }
}
...
<template>
    <div class="app-layout" id="moneyApp">
